  ...
  static void main() {
    ...
    for (int i = k; i < n; i++) {
      if (s[i] == '?') {
        ...
        must[i] = s[i] - '0';
        if (must[i - k] == 1 - must[i]) okay = false;
      }
    }
    ...
    const int cnt0 = count(must.begin(), must.begin() + k, 0);
    const int cnt1 = count(must.begin(), must.begin() + k, 1);
    print((cnt0 <= k / 2 and cnt1 <= k / 2) ? "YES" : "NO"), pline();
  }
