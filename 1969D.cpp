#include <bits/stdc++.h>
using namespace std;

#define int long long

#ifndef LOCAL
  #define debug(...)
#endif

using int64 = long long;
using ld = long double;

template<typename T>
using min_heap = priority_queue<T, vector<T>, greater<T>>;
template<typename T>
using max_heap = priority_queue<T, vector<T>, less<T>>;

template<typename T>
T read() {
  T x;
  cin >> x;
  return x;
}

template<typename T1, typename T2>
pair<T1, T2> read_pair() {
  pair<T1, T2> p;
  cin >> p.first;
  cin >> p.second;
  return p;
}

template<typename T>
vector<T> read_vec(const int n, const int index = 0) {
  vector<T> v(n + index);
  for (int i = index; i < n + index; i++) {
    v[i] = read<T>();
  }
  return v;
}

template<typename T>
vector<vector<T>> read_vec_vec(const int n, const int m, const int index_n = 0, const int index_m = 0) {
  vector<vector<T>> grid(n + index_n);
  for (int i = index_n; i < n + index_n; i++) {
    grid[i] = read_vec<T>(m, index_m);
  }
  return grid;
}

constexpr int kInf = 1e9 + 10;
constexpr int64 kInf64 = 1e15 + 10;
constexpr int kMod = 1e9 + 7;
constexpr int kMaxN = 2e5 + 10;

struct Item {
  int a;
  int b;
};

inline void solution() {
  const int n = read<int>();
  const int k = read<int>();

  vector<Item> items(n);
  for (auto &item : items) {
    item.a = read<int>();
  }

  for (auto &item : items) {
    item.b = read<int>();
  }

  sort(items.begin(), items.end(), [&](const Item &i1, const Item &i2) {
    return i1.b > i2.b;
  });

  int loss = 0;

  max_heap<int> useless;
  for (int i = 0; i < k; i++) {
    useless.push(items[i].a);
    loss += items[i].a;
  }

  int profit = 0;

  vector<int> delta(n);
  for (int i = 0; i < n; i++) {
    delta[i] = items[i].b - items[i].a;
    if (i >= k) profit += max(0LL, delta[i]);
  }

  int ans = max(0LL, profit - loss);
  for (int i = k; i < n; i++) {
    const auto [a, b] = items[i];

    useless.push(a);
    loss += a;
    loss -= useless.top();
    useless.pop();

    profit -= max(0LL, delta[i]);

    ans = max(ans, profit - loss);
  }

  cout << ans << '\n';
}

int32_t main() {
  ios_base::sync_with_stdio(false);
  cin.tie(nullptr);

  //freopen(".in", "r", stdin);
  //freopen(".out", "w", stdout);

  cout << fixed << setprecision(10);

  int testcases = 1;
  cin >> testcases;
  while (testcases--) {
    solution();
  }
}