...
void print_time(const int h, const int m) {
  cout << (h / 10) << (h % 10) << ':' << (m / 10) << (m % 10);
}

int reflect(map<int, int> &mp, const int time) {
  const int tens = time / 10, ones = time % 10;
  return mp[ones] * 10 + mp[tens];
}

void solve(const int testCase, map<int, int> &mp) {
  const int hours = nexti(), minutes = nexti();
  const int startH = nexti();
  const char c = nextc();
  const int startM = nexti();

  function<bool(int, int)> valid = [&](const int h, const int m) {
    return (h < hours and m < minutes);
  };

  for (int m = startM; m < minutes; m++) {
    if (valid(reflect(mp, m), reflect(mp, startH))) {
      print_time(startH, m), pline();
      return;
    }
  }
  for (int h = startH + 1; h < hours; h++) {
    for (int m = 0; m < minutes; m++) {
      if (valid(reflect(mp, m), reflect(mp, h))) {
        print_time(h, m), pline();
        return;
      }
    }
  }
  print_time(0, 0), pline();
}
...
int main() {
  ...
  mp[3] = 100, mp[4] = 100, mp[6] = 100, mp[7] = 100, mp[9] = 100;
