...
void solve(const int testCase) {
  const int n = nexti();
  vector<string> v = nextv<string>(2);
  for (int i = 0; i < n; i++) {
    if (v[0][i] == '1' or v[0][i] == '2') {
      v[0][i] = '1';
    } else {
      v[0][i] = '2';
    }
    if (v[1][i] == '1' or v[1][i] == '2') {
      v[1][i] = '1';
    } else {
      v[1][i] = '2';
    }
  }
  vector<vector<bool>> side(2, vector<bool>(n));
  vector<vector<bool>> middle(2, vector<bool>(n));
  side[0][0] = true;
  middle[1][0] = (v[0][0] == '2');
  for (int i = 1; i < n; i++) {
    for (int run = 0; run < 2; run++) {
      for (int level = 0; level < 2; level++) {
        const int other = (level + 1) & 1;
        side[level][i] = (v[level][i - 1] == '1' and side[level][i - 1]) or (v[level][i - 1] == '2' and middle[level][i - 1]);
        middle[level][i] = (v[other][i] == '2' and side[other][i]);
      }
    }
  }
  print(((side[1][n - 1] and v[1][n - 1] == '1') or (middle[1][n - 1] and v[1][n - 1] == '2') ? "YES" : "NO"));
  pline();
}
