...
inline void solution() {
  ...
  auto unite = [](const node &l, const node &r) {
    node ans;
    ans.len[0][0] = l.len[0][0] + r.len[0][0];
    ans.len[0][1] = max(l.len[0][0] + max(r.len[0][1], r.len[1][1]), l.len[0][1] + r.len[1][1]);
    ans.len[1][0] = max(l.len[1][1] + max(r.len[0][0], r.len[1][0]), l.len[1][0] + r.len[0][0]);
    ans.len[1][1] = l.len[1][1] + r.len[1][1];
    return ans;
  };

  auto apply = [](node &x, const node &p) {
    if (not p.flip) return;
    x.flip = not x.flip;
    swap(x.len[0][0], x.len[1][1]);
    swap(x.len[0][1], x.len[1][0]);
  };
  ...
  vector<node> a(n);
  for (node &x : a) {
    const bool i = (read::Char() == '7');
    x.len[i][i] = 1;
  }

  LazySegTree<node> st(a, unite, apply, activate);

  for (int i = 0; i < q; i++) {
    const string type = read::String();
    if (type == "count") {
      const node ans = st.query(0, n - 1);
      cout << max({ans.len[0][0], ans.len[0][1], ans.len[1][1]}) << '\n';
    } else {
      const int l = read::Int() - 1;
      const int r = read::Int() - 1;
      node updater;
      updater.flip = true;
      st.update(l, r, updater);
    }
  }
}
