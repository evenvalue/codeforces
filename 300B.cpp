...
void solve(const int testCase) {
  ...
  for (int x = 0; x < n; x++) {
    ...
    mp[d.leader(x)].push_back(x + 1);
  }
  vector<vector<int>> threes, twos;
  vector<int> ones;
  ...
  const int v1 = ones.size(), v2 = twos.size();
  if (v2 > v1) {
    cout << -1 << '\n';
    return;
  }
  for (const auto &v : threes) {
    print_space_vec(v);
  }
  for (int i = 0; i < v2; i++) {
    print_space_vec<int>({twos[i][0], twos[i][1], ones[i]});
  }
  for (int i = v2; i < v1; i += 3) {
    print_space_vec<int>({ones[i], ones[i + 1], ones[i + 2]});
  }
}
