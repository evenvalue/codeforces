...
void solve(const int testCase) {
  const int n = nexti(), k = nexti();
  vector<vector<int>> g(n + 1);
  for (int x = 1; x <= n; x++) g[0].push_back(x);
  vector<int> deg(n + 1, 1);
  for (int i = 0; i < n - 1; i++) {
    const int x = nexti(), y = nexti();
    g[x].push_back(y);
    g[y].push_back(x);
    deg[x]++, deg[y]++;
  }
  ...
  vector<int> cur_leaves = {0};
  int step = 0;
  while (step <= k and not cur_leaves.empty()) {
    vector<int> new_leaves;
    for (const int x : cur_leaves) {
      for (const int y : g[x]) {
        if (pushed[y]) continue;
        deg[y]--;
        if (deg[y] == 1 or deg[y] == 0) {
          new_leaves.push_back(y);
          pushed[y] = true;
        }
      }
      remain--;
