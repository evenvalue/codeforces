...
struct edge {
  ...
  explicit edge(const int w) : x(0), y(0), w(w) {}
  ...
  bool operator==(const edge &other) const {
    return (x == other.x and y == other.y and w == other.w);
  }
...
void solve(const int TestCase) {
  ...
  vector<int64> pref(n);
  for (int i = 1; i < n; i++) {
    const edge &e = edges[i - 1];
    pref[i] = pref[i - 1];
    pref[i] += (int64)d.size(e.x) * d.size(e.y);
    d.unite(e.x, e.y);
  }
  while (m--) {
    int q;
    cin >> q;
    const int dist = distance(edges.begin(), upper_bound(edges.begin(), edges.end(), edge(q)));
    cout << pref[dist] << ' ';
  }
  cout << '\n';
}
