...
void solve(const int TestCase) {
  ...
  for (int i = 0; i < 26; i++) {
    while (not l[i].empty() and not r[i].empty()) {
      ans.emplace_back(l[i].back(), r[i].back());
      l[i].pop_back(), r[i].pop_back();
    }
  }
  for (int i = 0; i < 26; i++) {
    while (not l[26].empty() and not r[i].empty()) {
      ans.emplace_back(l[26].back(), r[i].back());
      l[26].pop_back(), r[i].pop_back();
    }
    while (not r[26].empty() and not l[i].empty()) {
      ans.emplace_back(l[i].back(), r[26].back());
      l[i].pop_back(), r[26].pop_back();
    }
  }
  while (not l[26].empty() and not r[26].empty()) {
    ans.emplace_back(l[26].back(), r[26].back());
    l[26].pop_back(), r[26].pop_back();
  }
  cout << ans.size() << '\n';
