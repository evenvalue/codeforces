...
bool can_go(const range &r1, const range &r2) {
  return ((r2.l < r1.l and r1.l < r2.r) or (r2.l < r1.r and r1.r < r2.r));
}

void dfs(const vector<vector<int>> &g, vector<bool> &visit, const int x) {
  visit[x] = true;
  for (const int y : g[x]) {
    if (visit[y]) continue;
    dfs(g, visit, y);
...
bool call_dfs(const vector<vector<int>> &g, const int s, const int e) {
  const int n = g.size();
  vector<bool> visit(n);
  dfs(g, visit, s);
  return visit[e];
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (type == 1) {
      ...
      for (int y = 0; y < ranges.size(); y++) {
        if (can_go(r, ranges[y])) {
          g[x].push_back(y);
        }
        if (can_go(ranges[y], r)) {
          g[y].push_back(x);
        }
      }
      ranges.push_back(r);
    } else {
      const int s = nexti() - 1, e = nexti() - 1;
      print((call_dfs(g, s, e) ? "YES" : "NO")), pline();
    }
  }
}
