...
inline void solution() {
  ...
  vector<int64> c = read::Vec<int64>(n - 1);
  c.push_back(kInf64);
  ...
  auto unite = [](const node &l, const node &r) {
    node ans;
    const int64 brew = min(l.pos, r.neg);
    ans.wine = l.wine + r.wine + brew;
    ans.pos = min(l.pos - brew, r.cap) + r.pos;
    ans.neg = l.neg + min(l.cap, r.neg - brew);
    ans.cap = min(l.cap - min(l.cap, r.neg - brew), r.cap - min(l.pos - brew, r.cap));
    return ans;
  };

  auto leaf = [](const int a, const int b, const int64 c) {
    node ans;
    ans.wine = min(a, b);
    ans.pos = min(a - ans.wine, c);
    ans.neg = b - ans.wine;
    ans.cap = c - ans.pos;
    return ans;
  };

  SegTree<node> st(n, node(), unite);

  for (int i = 0; i < n; i++) {
    st.update(i, leaf(a[i], b[i], c[i]));
  }

  for (int qry = 0; qry < q; qry++) {
    const int i = read::Int() - 1;
    const int ai = read::Int();
    const int bi = read::Int();
    const int64 ci = read::Int64();

    st.update(i, leaf(ai, bi, ci));
    cout << st.query(0, n - 1).wine << '\n';
  }
}
