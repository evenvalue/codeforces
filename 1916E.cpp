...
inline void solution() {
  ...
  vector<int> colours = read::Vec<int>(n);
  vector<vector<int>> t(n);
  vector<vector<int>> stk(n + 1);

  vector<range> ranges(n);

  auto segtree = init_segtree(n);

  int64 ans = 1;

  int time = 0;
  function<void(int)> dfs = [&](const int x) {
    ranges[x].l = ranges[x].r = time++;

    if (not stk[colours[x]].empty()) {
      t[stk[colours[x]].back()].push_back(x);
    }
    stk[colours[x]].push_back(x);

    for (const int y : g[x]) {
      dfs(y);
      ranges[x].r = ranges[y].r;
    }

    for (const int y : t[x]) {
      segtree.update(ranges[y].l, ranges[y].r, {0, -1});
    }
    segtree.update(ranges[x].l, ranges[x].r, {0, 1});

    int mx = 1;
    for (const int y : g[x]) {
      const int cur = segtree.query(ranges[y].l, ranges[y].r).mx;
      ans = max(ans, mx * 1LL * cur);
      mx = max(mx, cur);
    }

    stk[colours[x]].pop_back();
  };
