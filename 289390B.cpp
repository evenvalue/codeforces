...
public:
  //constructor
  explicit dsu(const int n) : n(n), e(n, -1), groups(n), mx(n), mn(n) {
...
void solve(const int testCase) {
  ...
  for (int query = 0; query < q; query++) {
    ...
    if (s == "union") {
      ...
      const int x = d.leader(nexti() - 1);
      print_space_vec<int>({d.minimum(x) + 1, d.maximum(x) + 1, d.size(x)});
    }
  }
}
