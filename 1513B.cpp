...
void solve(const int TestCase, const vector<int> &f) {
  int n;
  cin >> n;
  vector<int> a(n);
  for (int &x : a) cin >> x;
  ...
  const int cnt = count(a.begin(), a.end(), mn);
  const int choose = ((cnt * 1LL * (cnt - 1))) % kMod;
  cout << (choose * 1LL * f[n - 2]) % kMod << '\n';
}
...
int main() {
  ...
  vector<int> f(kMaxN, 1);
  for (int i = 1; i < kMaxN; i++) {
    f[i] = (f[i - 1] * 1LL * i) % kMod;
  }
