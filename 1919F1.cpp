...
inline void solution() {
  ...
  auto unite = [](const node &l, const node &r) {
    node ans;
    ans.pos = r.pos + max(0LL, l.pos - r.neg);
    ans.neg = l.neg + max(0LL, r.neg - l.pos);
    ans.wine = l.wine + r.wine + min(l.pos, r.neg);
    return ans;
  };

  vector<node> leaf(n);
  for (int i = 0; i < n; i++) {
    (a[i] < b[i] ? leaf[i].neg : leaf[i].pos) = abs(a[i] - b[i]);
    leaf[i].wine = min(a[i], b[i]);
  }

  SegTree<node> st(leaf, unite);
  ...
  for (int qry = 0; qry < q; qry++) {
    ...
    node upd;
    (ai < bi ? upd.neg : upd.pos) = abs(ai - bi);
    upd.wine = min(ai, bi);
