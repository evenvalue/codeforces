...
inline void solution() {
  ...
  mint ans(0);
  for (int i = 0, count = 0; i < endpoints.size() - 1; i++) {
    const int sz = endpoints[i + 1] - endpoints[i];
    count += start.count(endpoints[i]);
    count -= end.count(endpoints[i]);
    ans += nCr(count, k) * mint(sz);
  }
