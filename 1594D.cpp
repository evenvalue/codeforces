...
template<typename T>
void print_space_vec(const vector<T> &a) {
  for (const T &x : a) {
    cout << x << ' ';
  }
  cout << '\n';
...
template<typename T>
void print_line_vec(const vector<T> &a) {
  for (const T &x : a) {
    cout << x << '\n';
  }
}
...
struct astro {
  ...
  astro &operator=(astro &other) {
    swap(crew, other.crew);
    swap(imp, other.imp);
    return *this;
  }

  bool operator==(const astro &other) const {
    return (crew == other.crew) and (imp == other.imp);
  }
};
...
void solve(const int testCase) {
  ...
  for (int i = 0; i < m; i++) {
    const int x = nexti() - 1, y = nexti() - 1;
    const string s = nexts();
    g[x].emplace_back(y, (s == "crewmate"));
    g[y].emplace_back(x, (s == "crewmate"));
  }
  vector<astro> type(n);
  vector<bool> visit(n);
  bool okay = true;
  function<pair<int, int>(int)> dfs = [&](const int x) {
    visit[x] = true;
    const astro cur = type[x];
    pair<int, int> ret = {cur.crew, cur.imp};
    for (const auto &[y, same] : g[x]) {
      if (visit[y]) {
        const astro make = {(cur.crew and same) or (cur.imp and not same), (cur.imp and same) or (cur.crew and not same)};
        okay &= (make == type[y]);
        continue;
      }
      type[y].crew = (cur.crew and same) or (cur.imp and not same);
      type[y].imp = (cur.imp and same) or (cur.crew and not same);
      const auto &[c, i] = dfs(y);
      ret.first += c, ret.second += i;
    }
    return ret;
  };
  int ans = 0;
  for (int x = 0; x < n; x++) {
    if (visit[x]) continue;
    type[x].crew = true;
    const auto &[c, i] = dfs(x);
    ans += max(c, i);
  }

  for (const auto &[c, i] : type) okay &= not (c & i);
  cout << (okay ? ans : -1) << '\n';
}
