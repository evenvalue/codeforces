...
inline void solution() {
  ...
  for (int x = 1; x <= mx; x++) {
    for (int i = 1; i * i <= x; i++) {
      ...
      if (other != i) factors[x].push_back(other);
    }
  ...
  vector<int> powers;
  for (int lcm = 1; lcm <= mx; lcm++) {
    auto &divisor = factors[lcm];

    divisor.push_back(mx + 1);
    for (int i = 0; i < factors[lcm].size() - 1; i++) {
      powers.push_back(pref[divisor[i + 1] - 1] - pref[divisor[i] - 1]);
    }
    divisor.pop_back();

    mint common(1);
    for (int i = 1; i < powers.size(); i++) {
      common *= mint(i).power(powers[i - 1]);
    }

    const int m = powers.size();
    const mint all = mint(m).power(powers.back());
    const mint exclude = mint(m - 1).power(powers.back());

    ans += common * (all - exclude);
    powers.clear();
  }
